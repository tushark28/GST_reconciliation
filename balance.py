...
print("")
print("5.) You have to Name the CGST column of both the files as \"CGST\" strictly caps sensitive")
print("")
...
# bookpath = input("Enter the Path of the BOOK: ").replace('\\','/')
m2bpath = "D:/Codes/Python/balancing/Master Sheet 2B.xlsx"
bookpath = "D:/Codes/Python/balancing/Master Sheet Books.xlsx"
# m2bpath = input("Enter the Path of the 2b File: ").replace('\\','/')
excel_df1 = read_excel(bookpath)
excel_df2 = read_excel(m2bpath)

# dropping any existing remarks
excel_df1 = excel_df1.drop('Remarks', axis=1, errors='ignore').dropna(axis=0,thresh=8)
excel_df2 = excel_df2.drop('Remarks', axis=1, errors='ignore').dropna(axis=0,thresh=8)
...
CGST_value1 = excel_df1["CGST"].tolist()
CGST_value2 = excel_df2["CGST"].tolist()
taxable_value_count = 0
for value in CGST_value1:
    try:
        excel_df1.at[taxable_value_count, "CGST"] = int(
            str(value).replace(',', '').split(".", 1)[0])
        taxable_value_count += 1
    except ValueError:
        excel_df1.at[taxable_value_count, "CGST"] = int("0")
        taxable_value_count += 1
...
for value in CGST_value2:
    try:
        excel_df2.at[taxable_value_count, "CGST"] = int(
            str(value).replace(',', '').split(".", 1)[0])
        taxable_value_count += 1
    except ValueError:
        excel_df2.at[taxable_value_count, "CGST"] = int("0")
        taxable_value_count += 1
...
cur.execute("SELECT GSTIN,\"Taxable Value\",CGST,SGST,\"Invoice No\" FROM books")
books = cur.fetchall()

# Selecting all the matched results from database and Setting remarks to match.
for book in books:
    cur.execute('SELECT "Invoice No" FROM twob WHERE GSTIN = ? AND "Taxable Value" = ? AND CGST = ? AND SGST = ? AND Remarks ="Not in Books" LIMIT 1',
